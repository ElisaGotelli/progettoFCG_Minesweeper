...
const char* window_title = "Interactive Control Panel";
...
{
     //AGGIUNTA: (a parte se la partita è finita) quando la finestra perde il focus il gioco viene stoppato in automatico e viene messa la schermata di pausa come se si fosse cliccato il pulsante PAUSA 
    if(!state.game_ended){
        state.pause();
        if(!state.first_move){
...
void handle_mouse_pressed (const sf::Event::MouseButtonPressed& mouse, sf::RenderWindow& window, State& state)
{
    ...
    //MODIFICATO: se il gioco è finito o in pausa
    if(state.game_ended || state.game_paused){
        if(mouse.button == sf::Mouse::Button::Left && state.stop_p.new_game_cb.cb_bounds.contains(mouse_pos) ) //se viene cliccato uno dei pulsanti presenti nelle schermata di pausa o fine gioco ed è stato cliccato con il pulsante sinistro del mouse il pulsante di nuova partita si fa il reset del gioco 
            state.reset(); 
